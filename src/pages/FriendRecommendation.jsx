...
import { 
  FiSend, FiUser, FiMessageSquare, FiUsers, 
  FiHeart, FiStar, FiMapPin, FiCoffee, FiBookOpen 
} from 'react-icons/fi';
...
function FriendRecommendation() {
  ...
  const [userProfile] = useState({
    ...
    topRatedPlaces: ['Bella Trattoria', 'Mountain View Trail', 'Blue Note Jazz Club'],
    friends: ['Sarah K.', 'Michael T.', 'Priya R.'],
    recentActivity: ['Reviewed Bella Trattoria (5★)', 'Checked in at Central Park', 'Liked The Coffee House']
  });
  const messagesEndRef = useRef(null);

  // Sample AI responses based on different recommendation strategies
  const aiResponses = {
    collaborative: [
      {
  ...
  const handleSubmit = (e) => {
    ...
    if (userInput.includes('similar') || userInput.includes('taste') || userInput.includes('restaurant') || userInput.includes('rating')) {
      responseType = 'collaborative';
    } else if (userInput.includes('interest') || userInput.includes('hobby') || userInput.includes('passion') || userInput.includes('like')) {
      responseType = 'interest';
    } else if (userInput.includes('mutual') || userInput.includes('friend') || userInput.includes('connection') || userInput.includes('network')) {
      responseType = 'graph';
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="hidden md:block">
            <div className="bg-base-200 rounded-lg shadow-lg p-4">
              <div className="flex items-center mb-4">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-sm opacity-70">Friend compatibility data</p>
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {userProfile.interests.map((interest, index) => (
                    <span key={index} className="px-2 py-1 bg-base-300 rounded-full text-xs">
                      {interest}
                    </span>
                  ))}
                </div>
              ...
              <div className="mb-4">
                <h3 className="font-semibold text-sm uppercase opacity-70 flex items-center">
                  <FiUsers className="mr-2 text-blue-500" /> Friends
                </h3>
                <ul className="mt-1">
                  {userProfile.friends.map((friend, index) => (
                    <li key={index} className="text-sm py-1">{friend}</li>
                  ))}
                </ul>
              </div>
              ...
              <div>
                ...
                <ul className="mt-1">
                  {userProfile.recentActivity.map((activity, index) => (
                    <li key={index} className="text-sm py-1 opacity-70">{activity}</li>
                  ))}
                </ul>
              </div>
            </div>
            ...
            <div className="bg-base-200 rounded-lg shadow-lg p-4 mt-4">
              <h2 className="text-xl font-bold mb-3">Our Recommendation Methods</h2>
              <div className="space-y-3">
                <div className="flex items-start">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="opacity-70">Finding friends with similar ratings and preferences</p>
                  </div>
                </div>
                <div className="flex items-start">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="opacity-70">NLP analysis of your reviews and interests</p>
                  </div>
                </div>
                <div className="flex items-start">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="opacity-70">Finding connections through your social network</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="md:col-span-2">
            <div className="bg-base-200 rounded-lg shadow-lg overflow-hidden h-[650px] flex flex-col">
              ...
              <div className="bg-[#ff5722] text-white p-4">
                <h1 className="text-2xl font-bold bebas-neue">Friend Recommendation AI</h1>
                <p className="opacity-90">Find friends based on shared interests and preferences</p>
              </div>
                ...
                {messages.map((message) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {message.sender === 'ai' && message.icon && message.type !== 'greeting' && message.type !== 'general' && (
                        <div className="flex items-center mb-2 text-sm">
                          <div className="mr-2">
                            {message.icon}
                          </div>
                          <span className="opacity-70">
                            {message.type === 'collaborative' && 'Collaborative Filtering Match'}
                            {message.type === 'interest' && 'Interest-Based Match'}
                            {message.type === 'graph' && 'Social Network Match'}
                          </span>
                        </div>
                      )}
                      ...
                      {message.friendDetails && (
                        <div className="mt-3 bg-base-100 rounded-lg p-3 shadow-sm">
                          <div className="flex items-center">
                            ...
                            />
                            <div className="ml-3">
                              <div className="font-bold">{message.friendDetails.name}</div>
                              <div className="text-xs flex items-center">
                                <span className="text-green-500 font-semibold">{message.friendDetails.compatibility}</span>
                                <span className="mx-1 opacity-70">compatibility match</span>
                              </div>
                            </div>
                          </div>
                          
                          <div className="mt-2">
                            <div className="text-xs font-semibold opacity-70 uppercase">Mutual Interests</div>
                            <div className="mt-1 flex flex-wrap gap-1">
                              {message.friendDetails.mutualInterests.map((interest, index) => (
                                <span key={index} className="px-2 py-0.5 bg-base-300 rounded-full text-xs">
                                  {interest}
                                </span>
                              ))}
                            </div>
                          ...
                          {message.friendDetails.mutualConnections && (
                            <div className="mt-2">
                              <div className="text-xs font-semibold opacity-70 uppercase">Mutual Connections</div>
              ...
              <form onSubmit={handleSubmit} className="border-t border-base-300 p-4 bg-base-100">
                ...
                <p className="text-xs mt-2 opacity-70">
                  Ask for recommendations based on shared interests, dining preferences, or mutual connections.
                </p>
              </form>
            </div>
            ...
            <div className="mt-4 bg-base-200 p-4 rounded-lg shadow text-sm">
              <h3 className="font-bold mb-1">How We Find Your Perfect Friends</h3>
              <p>We use collaborative filtering to match users with similar ratings, NLP analysis of reviews to find shared interests, and graph neural networks to analyze social connections. All recommendations are personalized based on your unique profile. </p>
            </div>
          </div>
        </div>
      </div>

// WordAssociationGraph.jsx
import React, { useEffect, useState, useRef } from 'react';
import * as d3 from 'd3';

const WordAssociationGraph = () => {
        ...
        <div className="space-y-3">
          {topAssociations.map((assoc, index) => (
            <div key={index} className="flex items-center">
              <div className="w-24 text-sm font-medium truncate pr-2" title={assoc.word}>
                {assoc.word}
              </div>
              <div className="flex-1 h-6 bg-gray-200 rounded-full overflow-hidden">
                ...
                >
                  ...
                  {(assoc.count / maxCount) > 0.3 && (
                    <span className="text-xs font-medium text-white">{assoc.count}</span>
                  )}
                </div>
              </div>
              ...
              {wordData.associations
                ...
                .map((assoc, index) => (
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-6 py-2 whitespace-nowrap text-sm font-medium text-gray-900">{assoc.word}</td>
                    <td className="px-6 py-2 whitespace-nowrap text-sm text-gray-500">{assoc.count}</td>
                  </tr>
                ))}
            </tbody>
          </table>
      ...
      {data && (
        <div>
          <div className="mb-4 bg-white p-4 rounded-lg shadow">
            ...
            <p className="text-sm text-gray-700">Sample Size: <span className="font-medium">{data.metadata.sampleSize.toLocaleString()}</span></p>

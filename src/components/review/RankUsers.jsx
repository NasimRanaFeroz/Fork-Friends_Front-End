...
const ReviewsPerYear = ({ onBack }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://192.168.37.177:5001/api/review/rank-users-by-reviews');
        if (!response.ok) {
              ...
              {data.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${yoyGrowth === 'N/A' ? 'text-gray-500' :

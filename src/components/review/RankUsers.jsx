...
const ReviewsPerYear = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:5001/api/review/rank-users-by-reviews');
        if (!response.ok) {
  ...
  const createChart = () => {
    ...
    svg.selectAll('bars')
      ...
      .attr('fill', d => activeYear === d.year ? 'url(#bar-hover-gradient)' : 'url(#bar-gradient)')
    ...
    data.forEach((d, i) => {
      if (i > 0) {
        ...
        const percentChange = ((change / prevYear.reviewCount) * 100).toFixed(1);
        const isPositive = change > 0;
        
        // Add arrow
        svg.append('path')
          .attr('d', d3.symbol().type(isPositive ? d3.symbolTriangle : d3.symbolTriangle).size(100))
          .attr('transform', `translate(${x(d.year) + x.bandwidth()/2}, ${y(d.reviewCount) - 35}) ${isPositive ? '' : 'rotate(180)'}`)
    ...
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="relative w-24 h-24">
          <div className="absolute top-0 w-full h-full rounded-full border-4 border-t-indigo-500 border-r-transparent border-b-transparent border-l-transparent animate-spin"></div>
          <div className="absolute top-2 left-2 w-20 h-20 rounded-full border-4 border-t-transparent border-r-indigo-400 border-b-transparent border-l-transparent animate-spin" style={{animationDirection: 'reverse', animationDuration: '1.5s'}}></div>
          <div className="absolute top-4 left-4 w-16 h-16 rounded-full border-4 border-t-transparent border-r-transparent border-b-indigo-300 border-l-transparent animate-spin" style={{animationDuration: '2s'}}></div>
        </div>
      </div>
    );
  }
  ...
  const highestYear = data.reduce((max, item) => item.reviewCount > max.count ? {year: item.year, count: item.reviewCount} : max, {year: 0, count: 0});
  const lowestYear = data.reduce((min, item) => (min.count === 0 || item.reviewCount < min.count) ? {year: item.year, count: item.reviewCount} : min, {year: 0, count: 0});

  return (
    <div className="container mx-auto px-4 py-8 bg-gray-50">
      <h1 className="text-4xl font-bold text-center mb-8 text-indigo-800 bg-clip-text bg-gradient-to-r from-indigo-700 to-purple-500">Reviews Per Year Analysis</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-indigo-500 transform transition-all duration-300 hover:scale-105 hover:shadow-lg">
          <h3 className="text-gray-500 text-sm font-medium uppercase tracking-wider">Total Reviews</h3>
          <p className="mt-2 text-3xl font-bold text-gray-900">{d3.format(',')(totalReviews)}</p>
        </div>
        
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-indigo-500 transform transition-all duration-300 hover:scale-105 hover:shadow-lg">
          <h3 className="text-gray-500 text-sm font-medium uppercase tracking-wider">Average Per Year</h3>
          <p className="mt-2 text-3xl font-bold text-gray-900">{d3.format(',')(avgReviews)}</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-green-500 transform transition-all duration-300 hover:scale-105 hover:shadow-lg">
          <h3 className="text-gray-500 text-sm font-medium uppercase tracking-wider">Peak Year</h3>
          <p className="mt-2 text-3xl font-bold text-gray-900">{highestYear.year}</p>
          <p className="text-sm text-gray-600">{d3.format(',')(highestYear.count)} reviews</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-yellow-500 transform transition-all duration-300 hover:scale-105 hover:shadow-lg">
          <h3 className="text-gray-500 text-sm font-medium uppercase tracking-wider">Lowest Year</h3>
          <p className="mt-2 text-3xl font-bold text-gray-900">{lowestYear.year}</p>
          <p className="text-sm text-gray-600">{d3.format(',')(lowestYear.count)} reviews</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-500 hover:shadow-xl">
        <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-indigo-500 to-purple-600">
          <h2 className="text-xl font-semibold text-white">Yearly Review Counts</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data.map((item, index) => {
                const percentOfTotal = ((item.reviewCount / totalReviews) * 100).toFixed(2);
                const prevYearReviews = index > 0 ? data[index-1].reviewCount : null;
                const yoyGrowth = prevYearReviews ? ((item.reviewCount - prevYearReviews) / prevYearReviews * 100).toFixed(2) : 'N/A';
                
                return (
                  <tr key={item.year} 
                    className={`hover:bg-indigo-50 transition-colors duration-150 ${activeYear === item.year ? 'bg-indigo-50' : ''}`}
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{d3.format(',')(item.reviewCount)}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{percentOfTotal}%</td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${
                      yoyGrowth === 'N/A' ? 'text-gray-500' : 
                      parseFloat(yoyGrowth) >= 0 ? 'text-green-600' : 'text-red-600'
                    }`}>
                      {yoyGrowth === 'N/A' ? yoyGrowth : (
                        <div className="flex items-center">
                          <span>{parseFloat(yoyGrowth) >= 0 ? '↑' : '↓'}</span>
                          <span className="ml-1">{Math.abs(parseFloat(yoyGrowth))}%</span>
                        </div>
                      )}
                    </td>
                  </tr>
      ...
      <div className="mt-8 bg-white rounded-lg shadow-lg p-6 transform transition-all duration-500 hover:shadow-xl">
        <h2 className="text-xl font-semibold text-gray-800 mb-4 bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">Review Count Trend Analysis</h2>
        <p className="text-gray-600 mb-4">
          ...
          The peak year was {highestYear.year} with {d3.format(',')(highestYear.count)} reviews.
          {data[data.length-1].reviewCount < data[data.length-2].reviewCount && 
            ` There was a decline in the most recent year (${data[data.length-1].year}) compared to the previous year.`
          }
        </p>
        
        {/* Growth rate visualization - enhanced sparkline */}
        <div className="mt-6">
          <h3 className="text-md font-medium text-gray-700 mb-3">Year-over-Year Growth Rate</h3>
          <div className="flex items-end h-24 space-x-2">
            {data.slice(1).map((item, index) => {
              const prevYearReviews = data[index].reviewCount;
              const growth = ((item.reviewCount - prevYearReviews) / prevYearReviews * 100);
              ...
              return (
                <div key={item.year} className="flex flex-col items-center group">
                  ...
                  <div 
                    className={`w-12 rounded-t-md ${growth >= 0 ? 'bg-gradient-to-t from-green-400 to-green-600' : 'bg-gradient-to-t from-red-400 to-red-600'} transition-all duration-500 ease-out transform hover:scale-110`} 
                    style={{
                      height: `${barHeight}%`, 

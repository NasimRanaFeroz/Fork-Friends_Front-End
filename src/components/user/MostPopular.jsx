...
const MultiChartVisualization = ({ onBack }) => {
  ...
  const drawPieChart = () => {
    ...
    const labelData = pie(data).map((d) => {
      ...
      const y =
        pos[1] + (midAngle > Math.PI / 2 && midAngle < Math.PI * 1.5 ? -5 : 5);
    ...
    const nameLabels = labelGroups
      .append("text")
      .attr("class", "pie-label")
      .attr(
        "transform",
        (d, i) => `translate(${labelData[i].x}, ${labelData[i].y - 8})`
      )
      .attr("dy", ".35em")
      .style("text-anchor", (d) => {
    ...
    const percentLabels = labelGroups
      ...
      .text(
        (d) =>
          `(${d3.format(".1%")(d.data.fans / d3.sum(data, (d) => d.fans))})`
      )
      .transition()
      .duration(600)
  ...
  const drawLineChart = () => {
    ...
    const y = d3
      ...
      .domain([
        0,
        d3.max(data, (d) => d3.max(d.trendData, (t) => t.value)) * 1.1,
      ])
      .range([height, 0]);

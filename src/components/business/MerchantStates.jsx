...
const MerchantStates = ({ onBack }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          setError(
            "Failed to load map data. The charts will still work without the map."
          );
        }
  ...
  const drawMapChart = () => {
      ...
      .on("mouseover", function (event, d) {
        ...
        if (stateDataMap[stateCode]) {
          ...
          tooltip
            ...
            .style("font-size", "12px")
            .text(
              `Merchants: ${stateDataMap[stateCode].count.toLocaleString()}`
            );
        }
      })
      .on("mouseout", function () {
    ...
    gradient.append("stop").attr("offset", "0%").attr("stop-color", "#bfdbfe");
  ...
  const drawCombinedChart = () => {
    ...
    const svg = d3
      .select(combinedChartRef.current)
      .append("svg")
      .attr("width", width + margin.left + margin.right)
      .attr("height", height + margin.top + margin.bottom)
      .append("g")
            ...
            {!usaGeoData && (
              <div className="flex items-center justify-center h-96">
                <div className="text-center text-amber-500">
                  ...
                  <p className="mt-4">
                    Map data could not be loaded. Other charts are still
                    available.
                  </p>
                </div>
              </div>

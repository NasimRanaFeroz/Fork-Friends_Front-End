...
const CommonMerchants = ({ onBack }) => {
  ...
  const drawRadialBarChart = () => {
    ...
    svg
      ...
      .attr("transform", (d) => {
        ...
        return `translate(${labelX},${labelY}) rotate(${
  ...
  const drawLineChart = () => {
    ...
    const yRating = d3.scaleLinear().domain([0, 5]).range([height, 0]);
  ...
  return (
    <div className="max-w-7xl mx-auto p-5 font-sans ">
      ...
      <h1
        className="text-3xl font-bold text-center text-gray-800 mb-8 opacity-0"
        ref={(el) => {
          if (el) {
            setTimeout(() => {
      ...
      {error && (
        <p
          className="text-center text-red-600 font-bold opacity-0"
          ref={(el) => {
            if (el) {
              setTimeout(() => {
      ...
      {!loading && !error && (
        <>
          <div
            className="mb-10 p-6 border border-gray-200 rounded-lg bg-white shadow-md opacity-0 transform translate-y-4"
            ref={(el) => {
              if (el) {
                setTimeout(() => {

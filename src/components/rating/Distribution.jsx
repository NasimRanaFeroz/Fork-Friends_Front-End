...
const Distribution = ({ onBack }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch('http://192.168.37.177:5001/api/rating/distribution');
  ...
  const renderPieChart = (data, totalCount) => {
    ...
    const tooltip = d3.select(pieChartRef.current)
      .append("div")
      .attr("class", "absolute bg-gray-800 text-white p-2 rounded shadow-lg text-sm z-10 pointer-events-none opacity-0 transition-opacity duration-300")

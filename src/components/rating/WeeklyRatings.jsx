...
const WeeklyRatingsAnalysis = ({ onBack }) => {
  ...
  const renderBarChart = () => {
    ...
    const colorScale = d3
      .scaleLinear()
      .domain([
        d3.min(weeklyData, (d) => d.count),
        d3.max(weeklyData, (d) => d.count),
      ])
      .range(["#818cf8", "#4f46e5"]);
      ...
      .on("mouseover", function (event, d) {
        ...
        `
          ...
          .style(
            ...
            `${
              event.pageX -
              barChartRef.current.getBoundingClientRect().left +
              10
            }px`
          )
          .style(
            "top",
            `${
              event.pageY - barChartRef.current.getBoundingClientRect().top - 40
            }px`
          );
      })
      ...
      .on("mouseover", function (event, d) {
        ...
        `
          ...
          .style(
            ...
            `${
              event.pageX -
              lineChartRef.current.getBoundingClientRect().left +
              10
            }px`
          )
          .style(
            ...
            `${
              event.pageY -
              lineChartRef.current.getBoundingClientRect().top -
              40
            }px`
          );
      })
  ...
  const renderAnimatedStats = () => {
    ...
    const minDay = weeklyData.reduce(
      (min, day) => (day.count < min.count ? day : min),
      weeklyData[0]
    );
    const weekendTotal = weeklyData
    ...
    setTimeout(() => {
      ...
      const weekendPercentageElement =
        document.getElementById("weekend-percentage");
      const weekdayPercentageElement =
        document.getElementById("weekday-percentage");

      if (totalRatingsElement)
        animateCounter(totalRatingsElement, totalRatings);
      if (avgDailyElement)
        animateCounter(avgDailyElement, Math.round(avgDailyRatings));
      ...
      if (weekendPercentageElement)
        animateCounter(
          weekendPercentageElement,
          Math.round((weekendTotal / totalRatings) * 100),
          1500,
          "",
          "%"
        );
      if (weekdayPercentageElement)
        animateCounter(
          weekdayPercentageElement,
          Math.round((weekdayTotal / totalRatings) * 100),
          1500,
          "",
          "%"
        );
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg relative">
      ...
      >
        <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-lg shadow-md p-4 text-white">
          ...
          <p id="total-ratings" className="text-3xl font-bold">
            0
          </p>
        </div>
        ...
        <div className="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg shadow-md p-4 text-white">
          ...
          <p id="avg-daily" className="text-3xl font-bold">
            0
          </p>
        </div>
        ...
        <div className="inline-flex rounded-md shadow-sm" role="group">
          ...
          >
            ...
            >
              <path
                fillRule="evenodd"
                d="M3 3a1 1 0 000 2h10a1 1 0 100-2H3zm0 4a1 1 0 000 2h6a1 1 0 100-2H3zm0 4a1 1 0 100 2h8a1 1 0 100-2H3zm10-4a1 1 0 100 2h1a1 1 0 100-2h-1z"
                clipRule="evenodd"
              />
        ...
        <table className="min-w-full divide-y divide-gray-300">
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {weeklyData.map((day, index) => {
              ...
              const avgDailyRatings = totalRatings / 7;
              const diffFromAvg = (
                ((day.count - avgDailyRatings) / avgDailyRatings) *
                100
              ).toFixed(1);
            ...
            <p className="text-gray-700">
              {weeklyData.length > 0 && (
                <>
                  ...
                  has the highest rating activity with{" "}
                  <span className="font-medium">
                    {d3.format(",")(
                      weeklyData.reduce(
                        (max, day) => (day.count > max.count ? day : max),
              ...
              {weeklyData.length > 0 && (
                <>
                  ...
                  {Math.max(...weeklyData.map((d) => d.count)) -
                    Math.min(...weeklyData.map((d) => d.count)) >
                  d3.mean(weeklyData, (d) => d.count) * 0.2
                    ? "significant"
                    : "relatively even"}{" "}
                  variation across the week, with a difference of{" "}
                  <span className="font-medium">
                    {d3.format(",")(
                      Math.max(...weeklyData.map((d) => d.count)) -
                        Math.min(...weeklyData.map((d) => d.count))
